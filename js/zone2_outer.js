...
const Zone2Outer = (() => {
    ...
    function applyBeatPulse(beatPulse, beatDetected) {
        // Text glow: proportional to beat pulse
        ...
        if (beatDetected) {
            ...
            if (current > 0.005) {
                document.documentElement.style.setProperty(
                    '--beat-bg-flash', (current * 0.9).toFixed(3)
                );
            }
        }
    }
